...
let students = [
	...
	//make array of just first names and print all, uses reduce()
	console.log(students.map(students => students.fname));
	//make array of full names and print separated by " " of who has egrade >= 80, use filter()
		//flipped made array of students with egrade >= 80 with filter, then map() those students full names
	console.log(students.filter(students => students.egrade >= 80).map(students => students.fname + " " + students.lname));
	//total avg of all final grade of all students, use map()
		//weights: a=40%, t=10%, e=50%
		//calc students grades separately then add to accumulator. at the end / by length of students to get avg
	//reduce at end and / length of students
	// console.log(students.reduce((totalGrade, what)=> totalGrade + (students.agrade * 0.40 + students.tgrade * 0.10 + students.egrade * 0.50)) / students.length);
		// close but doesnt work
	console.log(students.map(students =>  students.agrade * 0.40 + students.tgrade * 0.10 + students.egrade * 0.50).reduce((totalGrade,studentsGrade) => totalGrade + studentsGrade) / students.length);
	// calcs students grade first, then accumulates total grade and then avg at the end
