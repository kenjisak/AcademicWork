...
void podcastTest(){
  Podcast* pod = new Podcast(titles[0],hosts[0]);
  for (int i = 0; i < 5; ++i){
          stringstream ss;
          ss << "Episode Title: "<<i;
          string title = ss.str();
...
void destructorTest(){
  Podcast* pc[MAX_PODS];
  for (int j = 0; j < 3; ++j){
    pc[j]=new Podcast(titles[j],hosts[j]);
    for (int i = j*5; i <(j+1)*5; ++i){
      stringstream ss;
      ss << "Episode Title: "<<i;
      string title = ss.str();
      pc[j]->addEpisode(title, content[i]);
    }
  }
...
void deepCopyTest(){
  Podcast* pod = new Podcast(titles[1],hosts[1]);
  for (int i = 5; i < 10; ++i){
          stringstream ss;
          ss << "Episode Title: "<<i;
          string title = ss.str();
  ...
  Podcast* pod2 = new Podcast(*pod);
  delete pod;

  pod2->print();

  Episode* ep;
  pod2->getEpisode(3, &ep);
  ep ->play();
