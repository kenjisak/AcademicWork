...
bool Podcast::addEpisode(string title,string content){
    if (numEps <= MAX_EPS) {
        episodes[numEps] = new Episode(this->title,numEps , title,content);//new episode into array
        numEps++;
        return true;
...
bool Podcast::getEpisode(int index, Episode** ep){
    if (index <= numEps){
        *ep = episodes[index];//if index is valid then set return ep to the index ep
        return true;
    }else{
        return false;
    }
